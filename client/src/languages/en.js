export default {
  app_greeting: 'Hi from Web!',
  app_title_header: 'Auc',
  app_not_found: 'Page not found',
  app_lang_id: 'Indonesian',
  app_lang_en: 'English',
  app_popup_error_title: 'Something went wrong!',
  app_popup_error_message: 'Sorry, an error has occurred. Please try again later',
  app_popup_close_button_label: 'Close',

  nav_login: 'Login',
  nav_register: 'Register',
  nav_profile: 'Profile',
  nav_orderings: 'Orderings',
  nav_coupons: 'Coupons',
  nav_logout: 'Logout',

  empty_data: 'Empty',
  status_waiting: 'Waiting',
  status_failed: 'Failed',
  status_success: 'Ordered',
  back: 'Back',

  login_title: 'Login',
  login_title_forgot: 'Forgot Password',
  login_email: 'Email',
  login_email_placeholder: 'email_example@mail.com',
  login_password: 'Password',
  login_password_placeholder: 's3CRetp4sSW0rd',
  login_button: 'Login',
  login_button_forgot: 'Submit',
  login_forgot_pass_new_pass: 'Your New Password:',
  login_forgot_pass: 'Forgot Password?',
  login_forgot_pass_back: 'Back to login',
  login_forgot_pass_email_not_found: 'Email not found!',
  login_register_footer: "Don't have account? Register here",
  login_register_footer_link: 'here',
  login_validation: 'Must fill all!',
  login_wrong_credentials: 'Wrong email or password!',

  register_title: 'Register',
  register_fullname: 'Full Name',
  register_fullname_placeholder: 'Rendy Septio',
  register_dob: 'Date of Birth',
  register_email: 'Email',
  register_email_placeholder: 'email_example@mail.com',
  register_password: 'Password',
  register_password_placeholder: 's3CRetp4sSW0rd',
  register_location: 'Location',
  register_role: 'Role',
  register_role_placeholder: 'Choose Role',
  register_role_customer: 'Customer',
  register_role_business: 'Business',
  register_button: 'Register',
  register_login_footer: 'Have account? Login here',
  register_login_footer_link: 'here',
  register_fullname_validation: 'Name must be at least 3 characters long and lower equal than 255 characters!',
  register_email_validation: 'Must be a valid email address!',
  register_dob_validation: 'Date of Birth is required!',
  register_dob_age_validation: 'You must be at least 15+ years old to register!',
  register_password_validation: 'Password must be at least 6 characters long and lower equal than 20 characters!',
  register_role_valiation: 'Role must be filled!',
  register_email_has_taken: 'This email has already been taken!',

  product_card_price: 'person',

  home_title: 'Browse Products',
  home_best: 'Best Seller',
  home_create_new: 'Create New',
  home_search_placeholder: 'Search for products...',

  product_detail_title: 'Product Detail',
  product_detail_buy_btn: 'Buy Now',
  product_detail_desc: 'Description',
  product_detail_not_found: 'Product tidak ditemukan!',
  product_detail_used_coupons: 'Used Coupon',

  product_creation_title: 'Product Creation',
  product_creation_add_img_btn: 'Add Image',
  product_creation_chg_img_btn: 'Change Image',
  product_creation_rmv_img_btn: 'Delete Image',
  product_creation_title_label: 'Product Name',
  product_creation_location_label: 'Location',
  product_creation_variant_label: 'Variant',
  product_creation_name_label: 'Name',
  product_creation_price_label: 'Price (Rp.)',
  product_creation_add_btn: 'Add',
  product_creation_desc_label: 'Description',
  product_creation_save_btn: 'Save',
  product_creation_delete_btn: 'Delete',
  product_creation_title_validation: 'Title must be more than 5 characters and less equal than 255 characters!',
  product_creation_location_validation: 'Location must be more than 5 characters and less equal than 255 characters!',
  product_creation_description_validation:
    'Description must be more than 5 characters and less equal than 255 characters!',
  product_creation_variant_validation: 'At least have 1 variant!',
  product_creation_variant_empty_validation: 'Must fill all!',
  product_creation_variant_price_validation: 'Price must be more than 5,000 and less than 5,000,000!',
  product_creation_image_validation: 'At least have an image!',
  product_creation_save_success: 'Saved!',
  product_bought_by: 'Product has been bought',

  times: 'kali',

  payment_title: 'Payment',
  payment_previous_btn: 'Previous',
  payment_next_btn: 'Next',
  payment_phone: 'Phone Number',
  payment_address: 'Shipping Address',
  payment_finish_btn: 'Finish',
  payment_step_1_name: 'Order Forms',
  payment_step_2_name: 'Add Coupon',
  payment_step_3_name: 'Payment',
  payment_step_4_name: 'Summary',
  payment_total: 'Total',
  payment_prct_detail_page_title: 'Fill your shipping information below',
  payment_add_cpn_page_title: 'Select your preferred coupon',
  payment_add_cpn_page_prc_cut_none: 'Total Price',
  payment_add_cpn_page_prc_cut: 'Price Cut to',
  payment_add_cpn_page_prc_cut_validation: 'Coupon total is more than base price!',
  payment_payment_page_title: 'Select your payment method',
  payment_payment_transfer: 'Transfer',
  payment_payment_onsitepay: 'Onsite Payment',
  payment_summary_variant: 'Variant',
  payment_summary_total_payment: 'Total Payment',
  payment_summary_payment_method: 'Payment Method',
  payment_step_1_not_complete: 'Please fill the form first!',
  payment_step_3_not_complete: 'Please select payment method!',
  payment_step_complete: 'Ordering has been created!',

  orderings_title: 'Orderings',
  orderings_customer_name: 'Customer Name',
  orderings_customer_info: 'Customer Information',
  orderings_update_status_complete: 'Ordering Success',
  orderings_update_status_failed: 'Ordering Failed',

  coupons_title: 'Coupons',
  coupons_create_new: 'Create New',
  coupons_name: 'Name',
  coupons_price_cut: 'Price Cut (Rp.)',
  coupons_create: 'Create',
  coupons_name_validation: 'Name must be 5 characters minimum and 255 characters maximum!',
  coupons_price_cut_validation: 'Price cut must be Rp. 5,000 minimum and Rp. 200,000 maximum!',

  profile_title: 'Your Profile',
  profile_chg_img: 'Change Image',
  profile_delete_img: 'Remove',
  profile_general: 'General',
  profile_email: 'Email',
  profile_fullname: 'Full Name',
  profile_dob: 'Date of Birth',
  profile_save: 'Save',
  profile_customer: 'Customer',
  profile_business: 'Business',
  profile_generic_save_success: 'Save success!',
  profile_passwords: 'Passwords',
  profile_password_old: 'Old Password',
  profile_password_new: 'New Password',
  profile_password_confirm: 'Confirm Password',
  profile_password_fill_all: 'Must fill all!',
  profile_password_same_pass: 'New password and confirmation password must be same!',
  profile_password_old_pass_not_match: 'Wrong old password!',
  profile_password_success: 'Password changed successfully!',
  profile_location: 'Store Location',
};
